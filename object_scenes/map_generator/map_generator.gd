...
func _get_or_create_navigation_region(chunk_node: Node2D, nav_polygon: NavigationPolygon) -> NavigationRegion2D:
	...
	var nav_region: NavigationRegion2D = chunk_node.get_node_or_null("NavigationRegion2D")
	if nav_region == null:
...
func _bake_navigation_polygon(nav_region: NavigationRegion2D, chunk_node: Node2D):
	if not is_inside_tree(): return # Otherwise the we get error outputs when closing the game since blocks call 
	...
	# We apparently have to reassign the navigation_polygon after baking - otherweise the NavigationRegion does not use
	# the baked geometry (even if the debug shows them ... )
	nav_region.navigation_polygon = nav_polygon
...
func _load_blocks_from_persisted_chunk(chunk_node: Node2D, chunk_coord_string) -> void:
	...
	for grid_pos_string in map_chunk_blocks.keys():
		...
		var block_scene_path = BlockMappings.key_to_path[block_info.block_key]
		var block : Block = ObjectPool.get_from_pool(block_scene_path) # block_scene.instantiate() # // This takes about 4ms
		...
		_update_blocks_in_chunk(block_scene_path, blocks_in_chunk)
...
func _choose_block_scene_path(grid_pos: Vector2i, blocks_in_chunk: Dictionary) -> String:
	if block_types.size() > 0:
		var chosen_block = choose_block(grid_pos, blocks_in_chunk)
		if chosen_block != null:
			return chosen_block.resource_path
	return dirt_block.resource_path

func _add_new_block_to_chunk(block_scene_path: String, grid_pos: Vector2i, chunk_node: Node2D, blocks_in_chunk: Dictionary, map_chunk_blocks: Dictionary, chunk_coord_string) -> void:
	var block : Block = ObjectPool.get_from_pool(block_scene_path)
...
func _update_blocks_in_chunk(block_scene_path: String, blocks_in_chunk: Dictionary) -> void:
	if blocks_in_chunk.has(block_scene_path):
		blocks_in_chunk[block_scene_path] += 1
	else:
		blocks_in_chunk[block_scene_path] = 1
	...
	for generated_chunk_key in generated_chunks.keys():
		if not touched_chunks.has(generated_chunk_key):
			...
			for child in chunk_node.get_children():
				if child is Block:
					chunk_node.remove_child(child)
					ObjectPool.add_back_to_pool(child)
