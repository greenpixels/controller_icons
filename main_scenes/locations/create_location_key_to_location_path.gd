...
func _run():
	...
	var base_path: String = "res://main_scenes/locations/"
...
func _save_mapping(mapping: Dictionary):
	...
	var file_content: String = "class_name LocationMappings\n\n"
	file_content += "static var location_key_to_location_path_map: Dictionary = {\n"
	for key in mapping:
		file_content += "\t\"" + key + "\": \"" + mapping[key] + "\",\n"
	file_content += "}\n\n"
	...
	file_content += "static var location_path_to_location_key: Dictionary = {\n"
	for path in inverse_mapping:
		file_content += "\t\"" + path + "\": \"" + inverse_mapping[path] + "\",\n"
	file_content += "}\n"

	var file: FileAccess = FileAccess.open("res://main_scenes/locations/location_mappings.gd", FileAccess.WRITE)
	if file:
